...
const Navbar = () => {
  ...
  const navBg = isScrolled
    ? 'bg-[#fefae0]/90 backdrop-blur-md border-b border-[#d4a373]/20'
    : 'bg-transparent';
  const logoColor = isScrolled ? 'text-[#2C2420]' : 'text-[#fefae0]';

  return (
    <nav
      className={`fixed top-0 w-full z-[100] transition-all duration-500 px-6 md:px-12 ${navBg} ${
    ...
    >
      <div className='max-w-[1400px] mx-auto flex justify-between items-center'>
        ...
        <div className='grid items-center gap-3 cursor-pointer group'>
          <span
            className={`${logoColor} font-blosta text-2xl tracking-widest font-semibold transition-colors duration-500`}
          >
            Drip. & Dusk.
          </span>
        ...
        <div className='hidden md:flex items-center gap-12'>
          {[
            { name: 'Collections', id: '#Beverage' },
            { name: 'Visit Roastery', id: '#VisitUs' },
            { name: 'Our Story', id: '#OurStory' },
            { name: 'Contact', id: '#contact' },
          ].map((item) => (
            <a
              key={item.name}
              href={item.id} // Updated to dynamic ID
            ...
            >
              ...
              <span
                className={`absolute -bottom-1 left-0 w-0 h-px transition-all duration-300 group-hover:w-full ${
                  isScrolled ? 'bg-[#ccd5ae]' : 'bg-[#fefae0]'
                }`}
              />
            </a>
          ))}
        ...
        {/* ACTIONS */}
        <div
          className={`flex items-center gap-7 transition-colors duration-500 ${textColor}`}
        >
          <Search
            size={19}
            strokeWidth={1.5}
          ...
          <div className='relative cursor-pointer group'>
            ...
            />
            <span
              className={`absolute -top-1.5 -right-2.5 border text-[8px] font-bold w-4 h-4 rounded-full flex items-center justify-center transition-all duration-500
              ${
                isScrolled
